...
tournament_size_home = 10 # size of the tournament for reproductive selection. Within each tournament, the best reproduction_parents are selected for reproduction
mutation_rate_home = 0.10 # percentage of genes that on average will be mutated
...
offsprings_count_home = 100 # number of offspring to generate at each generation. They replace the worst offsprint_count derklings in the population

tournament_size_away = 10 # size of the tournament for reproductive selection. Within each tournament, the best reproduction_parents are selected for reproduction
mutation_rate_away = 0.10 # percentage of genes that on average will be mutated
...
# the selection is done in a lamba + m
offsprings_count_away = 100 # number of offspring to generate at each generation. They replace the worst offsprint_count derklings in the population

pop_size = 300 # number of derklings in the population, for each team, required to be multiple of 3.
...
mask = np.asarray([True for i in range(len(ObservationKeys))])
mask[8:20] = False
...
class Derk_Player:
    ...
    def __init__(self,state=None,random_state=False,
        ...
        if state is np.ndarray :
            ...
            self.state = np.zeros((1,mask_size))
            if random_state:
                self.state += np.random.choice([-1,0,1],size=(1,mask_size))
    ...
    def observation_to_state(self, observations):
        ...
        # state = observations
        return int((np.tanh(state)+1)/2*self.n_states)%self.n_states 
...
def evolutionary_strategy(players,rewards, tournament_size, mutation_rate , mutation_width, reproduction_parents, pop_size, offsprings_count):
    ...
    for i in range(offsprings_count):
        ...
        offspring = players[best[0]].crossover(players[best[1]],cross_over_rate=1,mutation_rate=mutation_rate)
...
for e in range(50):
    ...
    observation_n_masked = np.delete(observation_n, np.where(mask))
    # print('')
    
    while True:
        # do actions according to obeservations
        action_n_home = [players_home[i].actions(observation_n_masked[i],0.1) for i in range(pop_size)]
        ...
        observation_n_masked = [np.delete(observation_n[i], np.where(mask)) for i in range(pop_size*2)]
        # for i,player in enumerate(players_home):
        print(players_home[0].current_state)
        # print(np.max(players_home[0].q_table))
    ...
    for player in players_home:
        ...
        player.current_actions = np.asarray([1,len(ActionKeys)])
...
print("max reward home :{}".format(np.max(np.asarray(reward_home))))
print("max reward away :{}".format(np.max(np.asarray(reward_away))))
winner = players_home[sorted([i for i in range(pop_size)], key=lambda k: reward_n[k],reverse=True)[0]]
