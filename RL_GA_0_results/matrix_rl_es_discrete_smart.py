...
class Derk_Player:
    # create new player 
    # states is the table state that the agent learns from cosine similarity of observations
    ...
    def __init__(self,state=None,
        ...
        if state is np.ndarray :
            ...
            self.state = np.empty((1,mask_size))
        # init the q table for q learning
        if q_table is np.ndarray:
            self.q_table = np.copy(q_table)
        else:
            self.q_table = np.empty((1,len(actions_map)))
    ...
    def observation_to_state(self, observations):
        ...
        for i,state in enumerate(self.state):
            similarity = cosine(state,observations)
            if similarity > highest:
                highest = similarity
                highest_index = i
        if highest < 0.9:
            self.state = np.append(self.state,observations.reshape(1,self.state.shape[1]),axis=0)
            self.q_table = np.append(self.q_table,np.zeros(len(actions_map)).reshape(1,self.q_table.shape[1]),axis=0)
