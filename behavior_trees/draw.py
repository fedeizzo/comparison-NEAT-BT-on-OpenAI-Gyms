...
class BtDrawer:
    ...
    def get_hierarchy_pos(G, root=None, width=1., vert_gap = 0.2, vert_loc = 0, leaf_vs_root_factor = 0.5):
        ...
        def _hierarchy_pos(G, root, leftmost, width, leaf_dx = 0.2, vert_gap = 0.2, vert_loc = 0, 
                        x_center = 0.5, root_pos = None, 
                        leaf_pos = None, parent = None):
            ...
            if len(children)!=0:
                ...
                for child in children:
                    next_x += root_dx
                    root_pos, leaf_pos, new_leaves = _hierarchy_pos(G,child, leftmost+leaf_count*leaf_dx, 
                                        width=root_dx, leaf_dx=leaf_dx,
                                        vert_gap = vert_gap, vert_loc = vert_loc-vert_gap, 
                                        x_center=next_x, root_pos=root_pos, leaf_pos=leaf_pos, parent = root)
                    leaf_count += new_leaves

                leftmost_child = min((x for x,y in [leaf_pos[child] for child in children]))
                rightmost_child = max((x for x,y in [leaf_pos[child] for child in children]))
        ...
        if isinstance(G, nx.DiGraph):
            leaf_count = len([node for node in nx.descendants(G, root) if G.out_degree(node)==0])
        elif isinstance(G, nx.Graph):
            leaf_count = len([node for node in nx.node_connected_component(G, root) if G.degree(node)==1 and node != root])
        root_pos, leaf_pos, leaf_count = _hierarchy_pos(G, root, 0, width, 
                                                        leaf_dx=width*1./leaf_count, 
                                                        vert_gap=vert_gap, 
                                                        vert_loc = vert_loc, 
                                                        x_center = x_center)
        pos = {}
        for node in root_pos:
            pos[node] = (leaf_vs_root_factor*leaf_pos[node][0] + (1-leaf_vs_root_factor)*root_pos[node][0], leaf_pos[node][1]) 
    ...
    def draw(self):
        ...
        nx.draw_networkx_labels(self._bt_graph, positions, labels,
                                verticalalignment="center_baseline")  # draw node labels
